...
#Region "Other functions: Delete rename etc."
    ...
    ''' <summary>
    ''' Determine if folder exists on remote FTP site
    ''' </summary>
    ''' <param name="filename">Foldername full path</param>
    ''' <returns></returns>
    ''' <remarks>Note this only works for folder</remarks>
    Public Function FtpFolderExists(ByVal foldername As String) As Boolean

        'Try to obtain folder detail: if we get error msg containing "550"
        'the file does not exist
        Try
            Dim detail As FTPdirectory = ListDirectoryDetail(foldername)
            Return True
